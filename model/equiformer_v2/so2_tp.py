...
from .so3 import CoefficientMappingModule, SO3_Embedding
...
def rescale_tensor_product_weights(tp_weight_data, lmax):
    ...
    for instruction in fctp.instructions:
        tp_weight_data[
            instruction.i_out, :, instruction.i_in1, :, instruction.i_in2, :
...
class SO2_m_Convolution(torch.nn.Module):
    ...
    def __init__(self, m, sphere_channels, m_output_channels, lmax_list, mmax_list):
        ...
        ), "This implementation only supports single resolutions."
        assert self.m >= 0
        ...
        if self.m > 0:
            ...
            cg_array_m_neg = index_select_list(
                cg_array, [m_neg_index, m_pos_index, m_0_index]
            )  # (L_o, L_i, L_f)
    ...
    def forward(self, x_m, tp_weight, tp_bias=None):
        ...
        sub_tp_weight_pos = torch.einsum(
            "ijklmn, ijklmn -> ijkln", sub_tp_weight, cg_array_m_pos
        )
        ...
        if self.m > 0:
            ...
            sub_tp_weight_neg = torch.einsum(
                "ijklmn, ijklmn -> ijkln", sub_tp_weight, cg_array_m_neg
            )
            ...
            zero_tensor = torch.zeros(
                (sub_tp_weight.shape[0] - self.m_output_channels), device=tp_bias.device
            )
...
class SO2_Convolution_TensorProduct(torch.nn.Module):
    ...
    def __init__(
        ...
        ), "This implementation does not support radial functions."
        ...
        self.tp_weight = torch.nn.Parameter(
            torch.randn(
                (self.lmax + 1),
                self.m_output_channels,  # for L_o
                (self.lmax + 1),
                self.sphere_channels,  # for L_i
                (self.lmax + 1),
                1,  # for L_f
            )
        )
        ...
        self.tp_bias = torch.nn.Parameter(
            torch.zeros(self.m_output_channels)
        )  # only for L = 0
        ...
        if self.extra_m0_output_channels is not None:
            self.extra_m0_linear = torch.nn.Linear(
                (self.lmax + 1) * self.sphere_channels, self.extra_m0_output_channels
            )
